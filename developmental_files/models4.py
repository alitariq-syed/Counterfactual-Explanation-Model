...
#TODO 1. Debug target map creation
#TODO 2. debug mean filters at end of epoch... what happen when new epoch starts.... do class sums keep on adding or start from 0
#TODO 3. Save filter mean for each interpretable layer for post-training analysis
      ...
      if gt is not None:
          ...
          for f in range(x.shape[3]): #number of filters
              ...
              if self.templates is None:
                  self.templates = tf.Variable(initial_value=tf.zeros_like(filter_1),dtype=tf.dtypes.float32)
              else: 
                  self.templates[0:filter_1.shape[0]].assign(tf.zeros_like(filter_1))
              
#               ind = tf.gather_nd(
#     pos_templates[:,:,:,f], tf.where(f_class==gt_argmax),  batch_dims=0, name=None
# )
              for i in range(len(pos_indices)):
                  self.templates[pos_indices[i][0],:,:].assign(pos_templates[pos_indices[i][0],:,:,f])
              ...
              for i in range(len(neg_indices)):
                  self.templates[neg_indices[i][0],:,:].assign(tf.keras.layers.ReLU()(self.t_n))
              # for img in range(x.shape[0]):
              #     #for each filter, check if the batch_image and class are same, then stack positive template, else stack negative template
              #     if f_class == gt_argmax[img]:
         ...
         # loss = 1000*np.mean(abs(target_templates-x))
          ...
          loss=target_templates[0:filter_1.shape[0]]
