...
#%% 
def templates_return_pos(t):
  #t.append(x)
  return t  
def templates_return_neg(t):
  #t.append(x)
...
class MySubClassModel(Model):
  ...
  def compute_loss(self, x_masked, X_relu, positive_templates, activation_sums, class_sums, gt):
      if gt is not None:
          ...
          filter_means = tf.Variable(initial_value=tf.zeros(activation_sums.shape,dtype=tf.float32),dtype=tf.dtypes.float32)
          ...
          for f in range(x.shape[3]): #number of filters
              ...
              filter_means[f,:].assign(tf.clip_by_value(activation_sums[f,:]/class_sums, 0, 1000000.,))
          ...
          for f in range(x.shape[3]): #number of filters
              ...
              #templates
              target_templates.append(templates)
          target_templates = tf.transpose(tf.convert_to_tensor(target_templates),[1,2,3,0])    
