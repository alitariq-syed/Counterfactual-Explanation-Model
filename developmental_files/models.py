...
class MySubClassModel(Model):
  ...
  def __init__(self,base_model,args, num_classes=10):
    ...
    if args.interpretable:
        ...
        self.p_tp = self.alpha/self.n_square
  ...
  def compute_loss(self, x_masked, X_relu, positive_templates, activation_sums, class_sums, gt):
      if gt is not None:
          ...
          pos_filters = filter_class==gt_argmax.numpy()
          neg_filters = filter_class!=gt_argmax.numpy()
          ...
          for f in range(x.shape[3]): #number of filters
              ...
              for img in range(x.shape[0]):
                  #for each filter, check if the batch_image and class are same, then stack positive template, else stack negative template
                  if f_class == gt_argmax[img]:
                      templates.append(tf.math.exp(x[img,:,:,f]))#masked and then relu
                      # p_T.append(self.p_tp)
                  else:
                      templates.append(tf.math.exp(tf.keras.layers.ReLU()(X_relu[img,:,:,f] * self.t_n))) #x-->relu and then masked... investigate which is proper way
                      # p_T.append(self.p_tn)
                      
                  templates_2.append(tf.math.exp(x[img,:,:,f]))
                  p_T_2.append(self.p_tp)
                  
                  templates_3.append(tf.math.exp(tf.keras.layers.ReLU()(X_relu[img,:,:,f] * self.t_n)))
                  p_T_3.append(self.p_tn)
              #reshape templates
              #templates = tf.transpose(templates,[0,2,3,1])
              #t = tf.convert_to_tensor(templates)
              trace_xT   += tf.reduce_mean(templates)
              trace_xT_2 += tf.reduce_mean(templates_2)
              trace_xT_3 += tf.reduce_mean(templates_3)

          exp_trace = tf.math.exp(trace_xT)# this should be Z_t i think
          Z_t = exp_trace
          Z_t_2 =  tf.math.exp(trace_xT_2)
          Z_t_3 =  tf.math.exp(trace_xT_3)
