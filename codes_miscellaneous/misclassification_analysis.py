...
parser.add_argument('--alter_class', default = 5, type = np.int32) # alter class #misclassified classes 9-170
parser.add_argument('--analysis_class', default = 5, type = np.int32) # class for which images are loaded and analyzed
...
parser.add_argument('--alter_class_2', default = 5, type = np.int32) # alter class for 2nd example, 9, 170, 25, 125, 108
...
parser.add_argument('--dataset',default = 'mnist')#mnist, cifar10, CUB200, #cxr1000, #catsvsdogs, #VOC2010
...
parser.add_argument('--model',default = 'myCNN/')#myCNN, VGG16, resnet50,efficientnet
...
if logging: sys.stdout = open(file=weights_path+"/console_output_"+str(args.alter_class)+".txt", mode="w")    
...
if args.full_standard:
    ...
    if args.create_counterfactual_combined:
        ...
        model = tf.keras.Model(inputs=[base_model.input, fmatrix], outputs= [out,base_model.output, mean_fmap, modified_fmap,pre_softmax],name='VGG_base_model')
...
if args.test and args.train_using_builtin_fit_method and not args.create_counterfactual_combined:
    ...
    if args.dataset == 'CUB200' and official_split: test_gen = actual_test_gen
...
if (args.train_counterfactual_net or args.load_counterfactual_net):
    ...
    thresh=0.5
    x = tf.keras.layers.ThresholdedReLU(theta=thresh)(x)
    ...
    if args.train_counterfactual_net:
        ...
        #combined.summary()
        combined.load_weights(filepath=weights_path+'/counterfactual_combined_model_fixed_'+str(label_map[args.alter_class])+'_alter_class.hdf5')
...
if True:
    ...
    if args.dataset == 'CUB200': 
        ...
        test_gen_nopreprocess = actual_test_gen_nopreprocess#train_gen_nopreprocess[0]#actual_test_gen_nopreprocess
    gen=test_gen
    ...
    for k in range(batches):
        ...
        for i in range (len(x_batch_test)):
            ...
            if misclassification_analysis and False:
                if actual_img_ind != 218:#240 wrong, 216 correct
            ...
            combined.load_weights(filepath=weights_path+'/counterfactual_combined_model_fixed_'+str(label_map[alter_class])+'_alter_class.hdf5')

            statistical_analysis = False #global statistics for alter class images only
            if statistical_analysis:
                ...
                if args.dataset=='mnist':
                    ...
                    test_image_count = 30
                if filter_sum==30:#test_image_count[class_for_analysis]:
                   ...
                   # plt.plot(filter_magnitude_cf/(filter_histogram_cf+0.00001)), plt.show()
                    ...
                    mName = args.model[:-1]
                    #plt.ylim([0, np.max(c_mean_fmap)+1]), 
                    plt.plot(filter_histogram_cf), plt.ylim([0, np.max(filter_histogram_cf)+1]), plt.savefig(fname="./figs_for_paper/"+mName+"_filter_histogram_cf_alter_class_"+str(alter_class)+"_"+str(class_for_analysis)+"_train_set.png", dpi=None, bbox_inches = 'tight'), plt.show()
                    plt.plot(filter_magnitude_cf/max(filter_histogram_cf)), plt.ylim([0, np.max(filter_magnitude_cf/np.max(filter_histogram_cf))+1]), plt.savefig(fname="./figs_for_paper/"+mName+"_normalized_filter_magnitude_cf_alter_class_"+str(alter_class)+"_"+str(class_for_analysis)+"_train_set.png", dpi=None, bbox_inches = 'tight'), plt.show()
                    ...
                    plt.plot(filter_histogram_cf_thresholded), plt.ylim([0, np.max(filter_histogram_cf_thresholded)+1]),plt.savefig(fname="./figs_for_paper/"+mName+"_filter_histogram_thresholded_"+str(thresh)+"_cf_alter_class_"+str(alter_class)+"_"+str(class_for_analysis)+"_train_set.png", dpi=None, bbox_inches = 'tight'), plt.show()
            ...
            if generate_adversarial:
                ...
                adv_ex = sneaky_generate(170, x_batch_test[img_ind])
            ...
            #wrong prediction
            if np.argmax(pred_probs) != np.argmax(y_gt):
